...
const NeighborsPopup = ({ isOpen, neighbors, sourceHostname, onAddNeighbor, onClose }) => {
                    ...
                    {filteredNeighbors.length > 0 ? (
                        filteredNeighbors.map(neighbor => (
                            <li key={neighbor.ip + neighbor.interface}>
                                ...
                                <button className="add-neighbor-button" onClick={() => onAddNeighbor(neighbor)}>
                                    {t('sidebar.add')}
                                </button>
                            </li>
                        ))
                    ) : (
                        <li className="no-results">{t('neighborsPopup.noResults')}</li>
                    )}
                </ul>
            </div>
        </div>
