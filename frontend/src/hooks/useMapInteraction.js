...
import { calculateSmartPosition } from './useAutoLayout';
    ...
    const confirmNeighbor = useCallback(async (neighborGroup, sourceNodeId, setLoading, setError, isBatchOperation = false) => {
        ...
        if (isEndDevice) {
            setState(prev => {
                ...
                const position = calculateSmartPosition(sourceNode, 'Switch', prev.nodes);
        ...
        try {
            ...
            if (existingNode) {
                ...
                try {
                    ...
                    const deviceType = deviceResponse.data?.type || 'Unknown';
                    const position = calculateSmartPosition(sourceNode, deviceType, nodesRef.current);
                    ...
                    const position = calculateSmartPosition(sourceNode, 'Unknown', nodesRef.current);
